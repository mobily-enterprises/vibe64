...
- Session history route: `/app/project/[slug]/dashboard/history` is the single archive screen for issue sessions. It is reached from dashboard navigation as `Session History`, uses Vuetify tabs for `Completed` and `Abandoned`, stores the selected tab in `?tab=completed|abandoned`, and defaults missing or invalid tab state to `completed`.
