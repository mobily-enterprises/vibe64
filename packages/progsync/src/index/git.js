...
async function walkWorkingFiles(projectRoot, relativeDirectory, output = []) {
  const absoluteDirectory = absoluteProjectPath(
    projectRoot,
    relativeDirectory.replace(/\/$/u, "")
  );
  let entries;
  try {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    const relativePath = slashPath(`${relativeDirectory.replace(/\/$/u, "")}/${entry.name}`);
    if (entry.isDirectory()) {
      await walkWorkingFiles(projectRoot, relativePath, output);
    } else if (entry.isFile() && !entry.isSymbolicLink()) {
      output.push({
        relativePath,
        state: await readWorkingFile(projectRoot, relativePath)
      });
    } else {
      throw new ProgSyncError(
        "REGULAR_FILE_REQUIRED",
        `Owned auxiliary paths must contain only regular files: ${relativePath}`
      );
    }
  }
  return output;
...
async function readWorkingFiles(projectRoot, relativeDirectory) {
  return walkWorkingFiles(projectRoot, slashPath(relativeDirectory));
}

async function readGitFiles(projectRoot, baseCommit, relativeDirectory) {
  if (!baseCommit) {
    return [];
  }
  const prefix = slashPath(relativeDirectory);
  ...
  if (!result.ok) {
    throw new ProgSyncError(
      "GIT_FILE_READ_FAILED",
      `Cannot inspect owned auxiliaries below ${prefix} at Git base ${baseCommit}.`,
      { stderr: String(result.stderr || "").trim() }
    );
  }
  const paths = parseNulPaths(result.stdout).filter((relativePath) => (
    relativePath.startsWith(prefix)
  ));
  return Promise.all(paths.map(async (relativePath) => ({
    relativePath,
    state: await readGitFile(projectRoot, baseCommit, relativePath)
  })));
}
...
async function workingGitPaths(projectRoot) {
  await assertGitRepository(projectRoot);
