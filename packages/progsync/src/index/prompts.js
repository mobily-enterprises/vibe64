...
async function readPrompt(fileName) {
  return fs.readFile(path.join(PROMPT_ROOT, fileName), "utf8");
}

async function composeAtomicPrompt({
  allowedPathPrefixes = [],
  allowedPaths,
  capsule,
  mode,
  ...
  return `${basePrompt.trim()}\n\n${targetPrompt.trim()}\n\nRUN PARAMETERS\n\n` +
    ...
    (allowedPathPrefixes.length > 0
      ? `Allowed private auxiliary roots:\n${allowedPathPrefixes.map((filePath) => `- ${filePath}`).join("\n")}\n\n`
      : "") +
