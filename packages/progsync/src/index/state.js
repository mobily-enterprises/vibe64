...
function ownedFilesChanged(previous = [], current = []) {
  const byPath = (files) => new Map(files.map((entry) => [
    entry.relativePath,
    entry.state
  ]));
  ...
  return [...paths].some((relativePath) => fileChanged(
    before.get(relativePath) || { exists: false },
    after.get(relativePath) || { exists: false }
  ));
}

function classifyPair(snapshot) {
  const { P0, P1, I0, I1, A0 = [], A1 = [] } = snapshot;
  ...
  const implementationChanged = fileChanged(I0, I1) || ownedFilesChanged(A0, A1);
...
function snapshotSummary(snapshot) {
  return {
    ...
    A0: (snapshot.A0 || []).map((entry) => ({
      path: entry.relativePath,
      hash: sourceHash(entry.state),
      mode: entry.state.mode
    })),
    A1: (snapshot.A1 || []).map((entry) => ({
      path: entry.relativePath,
      hash: sourceHash(entry.state),
      mode: entry.state.mode
    }))
