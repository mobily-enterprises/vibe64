...
function gitCheckpointScript() {
  return shellScript([
    ...
    "set +e",
    "git -c safe.directory=\"$PWD\" ls-remote --exit-code origin \"$remote_ref\" >/dev/null 2>&1",
    "remote_ref_status=$?",
    "set -e",
    "if [ \"$remote_ref_status\" = \"0\" ]; then tracking_ref=\"refs/remotes/origin/$branch\"; timeout 120s git -c safe.directory=\"$PWD\" fetch origin \"$remote_ref:$tracking_ref\"; expected_placeholder_paths=\"$(printf '%s\\n' README.md vibe64.project.json vibe64.seed.json)\"; placeholder_paths=\"$(git -c safe.directory=\"$PWD\" ls-tree -r --name-only \"$tracking_ref\")\"; placeholder_commit_count=\"$(git -c safe.directory=\"$PWD\" rev-list --count \"$tracking_ref\")\"; placeholder_subject=\"$(git -c safe.directory=\"$PWD\" log -1 --format=%s \"$tracking_ref\")\"; seed_placeholder=no; if [ \"$placeholder_commit_count\" = \"1\" ] && [ \"$placeholder_subject\" = \"Add seed placeholder metadata\" ] && [ \"$placeholder_paths\" = \"$expected_placeholder_paths\" ] && git -c safe.directory=\"$PWD\" cat-file -e \"HEAD:vibe64.seed.json\" 2>/dev/null && [ \"$(git -c safe.directory=\"$PWD\" rev-parse \"$tracking_ref:vibe64.seed.json\")\" = \"$(git -c safe.directory=\"$PWD\" rev-parse \"HEAD:vibe64.seed.json\")\" ] && [ \"$(git -c safe.directory=\"$PWD\" rev-parse \"$tracking_ref:vibe64.project.json\")\" = \"$(git -c safe.directory=\"$PWD\" rev-parse \"HEAD:vibe64.project.json\")\" ]; then seed_placeholder=yes; fi; if ! git -c safe.directory=\"$PWD\" merge-base --is-ancestor \"$tracking_ref\" HEAD; then if [ \"$seed_placeholder\" != \"yes\" ]; then printf '[studio] origin/%s contains history that is not in this local checkout. Nothing was pushed.\\n' \"$branch\" >&2; printf '[studio] Reconcile the existing repository before publishing.\\n' >&2; exit 1; fi; if [ -n \"$(git -c safe.directory=\"$PWD\" status --porcelain=v1)\" ]; then printf '[studio] The matching seed placeholder was found, but the working tree is not clean. Nothing was pushed.\\n' >&2; exit 1; fi; printf '[studio] Matching Vibe64 seed placeholder found on origin/%s; preserving it as publication history.\\n' \"$branch\"; git -c safe.directory=\"$PWD\" merge --allow-unrelated-histories --strategy=ours -m \"Publish Vibe64 seed over placeholder\" \"$tracking_ref\"; fi; elif [ \"$remote_ref_status\" != \"2\" ]; then printf '[studio] Could not inspect origin/%s before publishing. Nothing was pushed.\\n' \"$branch\" >&2; exit 1; fi",
