...
function managedMariaDbServiceStartScript({
  ...
  return [
    ...
    "  local password=\"$1\"",
    "  shift",
    "  MYSQL_PWD=\"$password\" mariadb --no-defaults --skip-ssl --protocol=TCP --host=127.0.0.1 --port=\"$mariadb_port\" --user=root \"$@\"",
    "}",
    ...
    "}",
    "mariadb_root_open() {",
    "  env -u MYSQL_PWD mariadb --no-defaults --skip-ssl --protocol=TCP --host=127.0.0.1 --port=\"$mariadb_port\" --user=root \"$@\"",
    "}",
