...
function managedPreviewPolicyInstruction() {
  return [
    ...
    "- Codex's internal managed browser has its own application session, separate from the user's visible Preview. Use `vibe64-preview browser identity you` to sign it in as the Vibe64 user who authorized the turn, `vibe64-preview browser identity <existing-user-identifier>` to inspect another real application user by the identifier type that app supports, or `vibe64-preview browser identity guest` to sign it out. For authenticated verification, first run `vibe64-preview browser identity you`. If it fails because the authorizing Vibe64 user lacks a supported application email or user ID, immediately retry with the authorized explicit application identity supplied by the project or session context; report an authentication blocker only if both attempts fail. These commands never change the user's visible Preview.",
...
function promptSessionBriefing(contextInput = {}) {
  ...
  return [
    ...
    "Committed source contract files are `vibe64.project.json`, `vibe64.runtime-lock.json`, `vibe64.system.json`, optional `.vibe64/launcher`, `.vibe64/scripts`, `.vibe64/prompts`, and `.vibe64/project-knowledge`.",
