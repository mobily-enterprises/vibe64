...
function managedPreviewPolicyInstruction() {
  return [
    ...
    "- Codex's internal managed browser has its own application session, separate from the user's visible Preview. Use `vibe64-preview browser identity you` to sign it in as the Vibe64 user who authorized the turn, `vibe64-preview browser identity <existing-user-identifier>` to inspect another real application user by an identifier that app supports, or `vibe64-preview browser identity guest` to sign it out. For authenticated verification, first run `vibe64-preview browser identity you`. If the app cannot map the authorizing Vibe64 user, immediately retry with the authorized explicit application identity supplied by the project or session context; report an authentication blocker only if both attempts fail. These commands never change the user's visible Preview.",
