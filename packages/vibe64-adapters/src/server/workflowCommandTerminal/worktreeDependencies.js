...
function createWorktreeScript({
  ...
  return [
    ...
    `    git clone --reference-if-able "$VIBE64_GIT_CACHE_PATH" --single-branch --branch "$BASE_BRANCH" "$VIBE64_GIT_REMOTE_URL" ${quotedWorktreePath}`,
    "  else",
    ...
    ...(sessionUsesSourcePullRequest(session) ? [
      ...
      `mkdir -p "$(dirname ${quotedWorktreePath})"`,
      "if [ -n \"$VIBE64_GIT_CACHE_PATH\" ] && [ -d \"$VIBE64_GIT_CACHE_PATH\" ]; then",
      `  git clone --reference-if-able "$VIBE64_GIT_CACHE_PATH" --single-branch --branch "$CLONE_BASE_BRANCH" "$VIBE64_GIT_REMOTE_URL" ${quotedWorktreePath}`,
      "else",
