...
export const DATA_OVERVIEW_INSTRUCTIONS = [
  ...
  "Use each schema table's exact reference field everywhere in this file, never its qualifiedName. MySQL/MariaDB references are database-relative (bookings); PostgreSQL references retain the application schema (public.bookings). Never store the selected MySQL database name: it changes between sessions and environments. Every actor's table must be included in its tables list. Each table has one home; put genuinely shared concepts in their own actor and retain their relationships. Technical or unclassified tables may remain outside the groups and appear under Other tables.",
  "For an existing MySQL overview containing database-qualified names, convert it once to database-relative references. Confirm the old database prefix against the saved definition and current schema, then remove only that prefix from actor tables, memberships, reviewedTables, rings, position keys and mainRelationships. Preserve names, descriptions, positions, grouping and relationship choices; never discard missing tables or constraints to hide real schema changes. Check coverage afterward.",
  ...
  "Preserve existing authored groupings unless the task requires changing them. After a schema change, classify new tables, update renamed/removed references, then run vibe64-helper database refresh and vibe64-helper database overview --json to check coverage and warnings.",
  ...
  "Do not invent relationships or cardinalities; mainRelationships must use actual schema relationship reference fields. Vibe64 resolves these against the selected database and retains the actual endpoints and all physical relationships in the ERD; users can also show all overview connections. Do not read data rows or credentials merely to classify tables. Do not edit Vibe64 runtime artifacts."
].join("\n");
