<template>
  <div v-if="request?.workPlan?.text" class="d-flex align-center flex-wrap ga-1">
  ...
  <v-dialog v-model="planOpen" :activator="planActivator?.$el" max-width="880" scrollable aria-label="Working plan">
    <v-card rounded="xl">
  ...
  <v-alert v-if="actionable" variant="tonal" density="compact" :type="request.error ? 'warning' : 'info'" class="mb-2" role="status">
    ...
    <div v-if="followupNeedsAction" class="d-flex flex-wrap ga-1">
      <v-btn variant="text" min-height="48" :disabled="retrying" @click="$emit('retry')">
        {{ request.status.endsWith('_uncertain') ? 'Check delivery' : request.continuation === 'plan' ? 'Continue planning' : 'Retry review' }}
      </v-btn>
      <v-btn v-if="['review_pending', 'planning_pending'].includes(request.status)" variant="text" min-height="48" @click="$emit('skip')">
        {{ request.continuation === 'plan' ? 'Stop' : 'Skip review' }}
      </v-btn>
    </div>
  </v-alert>
</template>
...
const props = defineProps({ request: { type: Object, default: null }, active: Boolean, retrying: Boolean, busy: Boolean });
const emit = defineEmits(["retry", "skip", "implement"]);
const planOpen = ref(false);
const planActivator = ref(null);
const planReady = computed(() => props.request?.status === "done" && !props.request.error && props.request.workPlan?.status === "ready");
const implementLabel = computed(() => `Implement with ${props.request?.assignments?.code?.modelId || "Code"}`);
const planStage = computed(() => {
  const request = props.request;
  if (request?.task === "deslop") return request.status === "sent" ? "Deslopping" : "Plan needs updating";
  if (request?.status === "sent") return request.resolvedMode === "code" ? "Coding" : "Planning";
