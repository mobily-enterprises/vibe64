...
test("Vibe64 project service resolves and materializes JSKIT dev runtime config", async () => {
  await withTemporaryRoot(async (targetRoot) => {
    ...
    const publishableKeyRecord = runtimeConfig.view.records.find((record) => record.key === "JSKIT_AUTH_SUPABASE_PUBLISHABLE_KEY");
    ...
    assert.equal(env.JSKIT_AUTH_SUPABASE_URL, "https://devref.supabase.co");
